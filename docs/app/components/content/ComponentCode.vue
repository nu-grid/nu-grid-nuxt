...
const castMap: Record<string, Cast> = {
  ...
  'DateRange': {
    get: (args: { start: CastDateValue; end: CastDateValue }) => ({
      start: new CalendarDate(...args.start),
      end: new CalendarDate(...args.end),
    }),
...
function mapKeys(obj: object, parentKey = ''): any {
  return Object.entries(obj || {})
    .flatMap(([key, value]: [string, any]) => {
      ...
      return !props.ignore?.includes(fullKey) && !props.hide?.includes(fullKey)
        ? fullKey
        : undefined
    })
    .filter(Boolean)
  ...
  return keys.map((key: string) => {
    ...
    const items = propItems.length
      ...
      : prop?.type === 'boolean' || prop?.type === 'boolean | undefined'
        ...
        : Object.keys(componentTheme?.variants?.[key] || {})
            .filter((variant) => {
...
::${camelName}`
    ...
    const proseProps = Object.entries(componentProps)
      .filter(
        ([key, value]) =>
          value !== undefined && value !== null && value !== '' && !props.hide?.includes(key),
      )
...
`
    if (props.externalTypes?.length) {
      const removeArrayBrackets = (type: string): string =>
        type.endsWith('[]') ? removeArrayBrackets(type.slice(0, -2)) : type
...
`
    ...
    for (const [i, key] of props.external.entries()) {
      const cast = props.cast?.[key]
      const value = cast
        ? castMap[cast]!.template(componentProps[key])
        : json5.stringify(componentProps[key], null, 2)?.replace(/,([ |\t\n]+[}|\]])/g, '$1')
...
<template>
  ...
  for (const [key, value] of Object.entries(componentProps)) {
    ...
    if (typeof value === 'boolean') {
      ...
      const parsedValue = !props.external?.includes(key)
        ? json5.stringify(value, null, 2).replace(/,([ |\t\n]+[}|\])])/g, '$1')
        : key
...
const { data: ast } = await useAsyncData(
  `component-code-${name}-${hash({ props: componentProps, slots: props.slots, external: props.external, externalTypes: props.externalTypes, collapse: props.collapse })}`,
  async () => {
    if (!props.prettier) {
        ...
        <template v-for="option in options" :key="option.name">
          ...
          >
            ...
            <UInput
              v-else
              :type="
                option.type?.includes('number') && typeof getComponentProp(option.name) === 'number'
                  ? 'number'
                  : 'text'
              "
              :model-value="getComponentProp(option.name)"

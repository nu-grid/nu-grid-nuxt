...
Set default options for all columns using the `columnDefaults` prop:
...
|--------|------|---------|-------------|
| `resize` | `boolean` | `false` | Enable column resizing by default |
| `reorder` | `boolean` | `false` | Enable column reordering by default |
| `wrapText` | `boolean` | `false` | Enable text wrapping by default |
| `sortIcons` | `object` | See below | Default sort icon configuration |
| `menu` | `object` | See below | Default column menu configuration |
| `lookup` | `object` | See below | Default lookup configuration |
...
```ts
{
  ...
  unsortedHover: true,  // Show unsorted icon on hover
  position: 'edge',     // 'edge' | 'inline'
}
```
...
```ts
{
  preset: 'minimal',        // 'minimal' | 'full'
  visibilityToggle: true,   // Show column visibility toggle
  button: {
...
Default settings for all `cellDataType: 'lookup'` columns:
...
### Overriding Defaults Per Column

Column-level settings override the defaults:
...
const columns: NuGridColumn<User>[] = [
  {
    accessorKey: 'id',
    header: 'ID',
    enableResizing: false,  // Override default resize: true
  },
  {
    accessorKey: 'name',
    header: 'Name',
    // Uses default resize: true from columnDefaults
  },
]
```
...
Define aggregate calculations for columns to display in group summaries and grand totals:
...
|--------|------|-------------|
| `aggregate` | `'sum' \| 'avg' \| 'count' \| 'min' \| 'max' \| Function` | Built-in aggregate type or custom function |
| `format` | `(value, context) => string` | Format the calculated value for display |
...
See [Grouping](/layout/grouping#summaries-configuration) for enabling summaries at the grid level.

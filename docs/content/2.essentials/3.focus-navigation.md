...
| `autoFocus` | `boolean` | `false` | Automatically focus first row/cell when data loads |

## Auto Focus on Load

Enable `autoFocus` to automatically focus the first row/cell when data loads:
...
<template>
  <NuGrid
    :data="data"
    :columns="columns"
    :focus="{ mode: 'cell', autoFocus: true }"
  />
</template>
```

This is useful for:
- Keyboard-first interfaces where users expect immediate navigation
- Master-detail layouts that should show details on load

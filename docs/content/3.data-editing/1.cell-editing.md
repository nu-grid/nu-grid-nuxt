---
title: Cell Editing
description: Enable inline cell editing in NuGrid.
navigation:
  icon: i-lucide-pencil
---

NuGrid provides powerful inline cell editing with support for various data types, validation, and custom editors.

:component-example{name="nu-grid-editing-example"}

## Enabling Editing

Enable editing with the `editing` prop:

```vue
<template>
  <!-- Boolean shorthand - enable with defaults -->
  <NuGrid :data="data" :columns="columns" :editing="true" />

  <!-- Object form for customization -->
  <NuGrid
    :data="data"
    :columns="columns"
    :editing="{
      startClicks: 'double',
      startKeys: ['enter', 'f2'],
    }"
  />
</template>
```

::callout{icon="i-lucide-lightbulb" color="primary"}
Using `:editing="true"` is equivalent to `:editing="{ enabled: true }"` with all default settings.
::

## Editing Options

Configure editing behavior:

```ts
interface NuGridEditingOptions {
  enabled: boolean
  startClicks: 'none' | 'single' | 'double'
  startKeys: ('enter' | 'f2' | 'bs' | 'alpha' | 'numeric')[]
  enterBehavior: 'default' | 'moveDown' | 'moveCell'
  defaultEditors: Record<string, Component>
}
```

### Options Reference

| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `enabled` | `boolean` | `false` | Enable editing |
| `startClicks` | `'none' \| 'single' \| 'double'` | `'double'` | Click to start editing |
| `startKeys` | `array` | `['enter', 'f2', 'bs', 'alpha', 'numeric']` | Keys that start editing |
| `enterBehavior` | `'default' \| 'moveDown' \| 'moveCell'` | `'default'` | What Enter does after saving |
| `defaultEditors` | `Record<string, Component>` | `undefined` | Override default editors by cell data type |

### Start Keys

| Key | Description |
|-----|-------------|
| `enter` | Enter key starts editing |
| `f2` | F2 key starts editing |
| `bs` | Backspace/Delete clears and starts editing |
| `alpha` | Any letter key replaces and starts editing |
| `numeric` | Any number key replaces and starts editing |

## Per-Column Editing

Enable or disable editing per column:

```ts
const columns: NuGridColumn<Product>[] = [
  {
    accessorKey: 'id',
    header: 'ID',
    enableEditing: false,  // ID is not editable
  },
  {
    accessorKey: 'name',
    header: 'Name',
    enableEditing: true,   // Explicitly enable
  },
  {
    accessorKey: 'price',
    header: 'Price',
    // Editing enabled by default when grid editing is on
  },
]
```

## Cell Data Types

Use cell data types for appropriate editors:

```ts
const columns: NuGridColumn<Product>[] = [
  {
    accessorKey: 'name',
    header: 'Name',
    cellDataType: 'text',      // Text input editor
  },
  {
    accessorKey: 'price',
    header: 'Price',
    cellDataType: 'number',    // Number input editor
  },
  {
    accessorKey: 'active',
    header: 'Active',
    cellDataType: 'boolean',   // Checkbox editor
  },
  {
    accessorKey: 'category',
    header: 'Category',
    cellDataType: 'selection', // Dropdown editor
    cellDataTypeOptions: {
      options: ['Electronics', 'Clothing', 'Food'],
    },
  },
]
```

## Handling Value Changes

Listen to cell value changes:

```vue
<script setup lang="ts">
function onCellValueChanged(event) {
  console.log('Changed:', {
    rowId: event.row.id,
    column: event.column.id,
    oldValue: event.oldValue,
    newValue: event.newValue,
  })

  // Persist to server
  await updateRecord(event.row.original.id, {
    [event.column.id]: event.newValue,
  })
}
</script>

<template>
  <NuGrid
    :data="data"
    :columns="columns"
    :editing="{ enabled: true }"
    @cell-value-changed="onCellValueChanged"
  />
</template>
```

## Editing Events

NuGrid emits several editing-related events:

```vue
<script setup lang="ts">
function onCellEditingStarted(event) {
  console.log('Started editing:', event.row.id, event.column.id)
}

function onCellEditingCancelled(event) {
  console.log('Cancelled editing:', event.row.id, event.column.id)
}

function onCellValueChanged(event) {
  console.log('Value changed:', event.oldValue, '->', event.newValue)
}
</script>

<template>
  <NuGrid
    :data="data"
    :columns="columns"
    :editing="{ enabled: true }"
    @cell-editing-started="onCellEditingStarted"
    @cell-editing-cancelled="onCellEditingCancelled"
    @cell-value-changed="onCellValueChanged"
  />
</template>
```

## Overriding Default Editors

Override the built-in editor for any cell data type using `defaultEditors`:

```vue
<script setup lang="ts">
import MyNumberEditor from '~/components/MyNumberEditor.vue'
import MyCurrencyEditor from '~/components/MyCurrencyEditor.vue'
</script>

<template>
  <NuGrid
    :data="data"
    :columns="columns"
    :editing="{
      enabled: true,
      defaultEditors: {
        number: MyNumberEditor,
        currency: MyCurrencyEditor,
        percentage: MyPercentageEditor,
      }
    }"
  />
</template>
```

All columns with `cellDataType: 'number'` will now use `MyNumberEditor` instead of the built-in number editor.

::callout{icon="i-lucide-lightbulb" color="primary"}
This is useful when you want a consistent custom editor across all columns of a particular type without specifying it on each column.
::

## Custom Cell Editors

### Per-Column Custom Editor

Override the editor for a specific column using the `editor` property:

```ts
import SliderEditor from '~/components/SliderEditor.vue'

const columns: NuGridColumn<Product>[] = [
  {
    accessorKey: 'discount',
    header: 'Discount %',
    cellDataType: 'percentage',
    // Custom editor just for this column
    editor: {
      component: SliderEditor,
      props: { min: 0, max: 100, step: 5 }
    },
  },
]
```

### Inline Editor Component

You can also define the editor inline:

```vue
<script setup lang="ts">
const columns: NuGridColumn<Product>[] = [
  {
    accessorKey: 'rating',
    header: 'Rating',
    editor: defineComponent({
      props: ['modelValue'],
      emits: ['update:modelValue'],
      setup(props, { emit }) {
        return () => h('div', { class: 'flex gap-1' }, [
          ...Array.from({ length: 5 }, (_, i) =>
            h('button', {
              class: i < props.modelValue ? 'text-yellow-400' : 'text-gray-300',
              onClick: () => emit('update:modelValue', i + 1),
            }, '★')
          ),
        ])
      },
    }),
  },
]
</script>
```

### Editor Priority

Editors are resolved in this order:
1. Column-level `editor` property (highest priority)
2. Grid-level `defaultEditors` for the cell data type
3. Built-in editor for the cell data type
4. Default text editor (fallback)

## Keyboard Navigation During Edit

| Key | Action |
|-----|--------|
| `Enter` | Save (behavior depends on `enterBehavior` option) |
| `Shift + Enter` | Save and navigate opposite to Enter direction |
| `Tab` | Save and move to next cell |
| `Shift + Tab` | Save and move to previous cell |
| `Escape` | Cancel editing |
| `Arrow keys` | Save and navigate (configurable) |

## Enter Key Behavior

Control what happens when Enter is pressed during editing with the `enterBehavior` option:

| Value | Enter | Shift+Enter |
|-------|-------|-------------|
| `'default'` | Save and stay on cell | Same as Enter |
| `'moveDown'` | Save and move down | Save and move up |
| `'moveCell'` | Save and move to next cell | Save and move to previous cell |

```vue
<template>
  <!-- Excel-like: Enter moves down -->
  <NuGrid
    :data="data"
    :columns="columns"
    :editing="{
      enabled: true,
      enterBehavior: 'moveDown',
    }"
  />

  <!-- Tab-like: Enter moves to next cell -->
  <NuGrid
    :data="data"
    :columns="columns"
    :editing="{
      enabled: true,
      enterBehavior: 'moveCell',
    }"
  />
</template>
```

::callout{icon="i-lucide-lightbulb" color="primary"}
For textarea editors, plain Enter still creates a new line. Use `Cmd/Ctrl+Enter` or `Shift+Enter` to save and navigate.
::

## Preventing Edit

Cancel editing programmatically by returning `false` from a handler:

```vue
<script setup lang="ts">
function onCellEditingStarted(event) {
  // Prevent editing locked rows
  if (event.row.original.locked) {
    return false
  }
}
</script>

<template>
  <NuGrid
    :data="data"
    :columns="columns"
    :editing="{ enabled: true }"
    @cell-editing-started="onCellEditingStarted"
  />
</template>
```

## Example: Editable Grid with Auto-Save

```vue
<script setup lang="ts">
const toast = useToast()
const saving = ref(false)

async function onCellValueChanged(event) {
  saving.value = true

  try {
    await api.updateItem(event.row.original.id, {
      [event.column.id]: event.newValue,
    })

    toast.add({
      title: 'Saved',
      description: `Updated ${event.column.id}`,
      color: 'success',
    })
  } catch (error) {
    toast.add({
      title: 'Error',
      description: 'Failed to save changes',
      color: 'error',
    })

    // Revert the change
    event.row.original[event.column.id] = event.oldValue
  } finally {
    saving.value = false
  }
}
</script>

<template>
  <NuGrid
    :data="data"
    :columns="columns"
    :editing="{ enabled: true, startClicks: 'double' }"
    :loading="saving"
    @cell-value-changed="onCellValueChanged"
  />
</template>
```

## Next Steps

::card-group
  :::card
  ---
  icon: i-lucide-plus
  title: Add New Rows
  to: /data-editing/add-row
  ---
  Enable adding new rows to the grid.
  :::

  :::card
  ---
  icon: i-lucide-shield-check
  title: Validation
  to: /data-editing/validation
  ---
  Validate cell values with schemas.
  :::
::

...
Groups render in the `'group'` (or `'splitgroup'`) layout. A grid that starts with `grouping` set and
no `layout.mode` uses `'group'` automatically. If grouping can start empty and be switched on later,
set `:layout="{ mode: 'group' }"` yourself: the layout is chosen when the grid is created, and a flat
(`'div'`) grid shows grouped rows ungrouped, with a warning in development. Until grouping is set, a
`'group'` or `'splitgroup'` grid shows its rows flat, under one set of column headers.

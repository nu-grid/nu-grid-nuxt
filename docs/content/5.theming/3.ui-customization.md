...
The `ui` prop follows Nuxt UI's rules on every slot:

- **A class value is merged** with the theme's classes for that slot. Where both set the same
  property, yours wins.
- **A function replaces them.** It receives the theme's classes as a string, and whatever it returns
  is used instead.
...
<template>
  <NuGrid
    ...
    :columns="columns"
    :ui="{
      // merged: the theme's th classes plus these
      th: 'bg-elevated',
      // replaced: only these classes, built from the defaults you're given
      td: (defaults) => defaults.replace('text-muted', 'text-default') + ' py-1',
    }"
  />
</template>
```

The same rules apply to slots in `app.config` `nuGrid.ui` (see [Themes](/theming/themes)).

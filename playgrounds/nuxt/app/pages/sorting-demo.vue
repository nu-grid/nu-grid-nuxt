...
<template>
  <DemoLayout id="sorting-demo" title="Sorting Engine Demo" info-label="About NuGrid Sorting">
    <!-- Status Indicators -->
    <template #status>
    ...
    <template #controls>
      ...
      <div class="border-default bg-elevated/30 rounded-lg border p-3">
        ...
        <pre class="bg-default/50 max-h-32 overflow-auto rounded p-2 text-xs">{{
    ...
    <template #info>
      ...
      <ul class="text-muted mb-3 list-inside list-disc space-y-1 text-sm">
        <li>
          <strong>sortAccessor (string):</strong> Status column displays status text but sorts by
          statusSort number
        </li>
        <li>
          <strong>sortAccessor (function):</strong> Priority column sorts by priorityWeight
          (Critical=4 > High=3 > Medium=2 > Low=1)
        </li>
        <li>
          <strong>Alphanumeric:</strong> Version and Mixed columns handle v1.0 &lt; v2.0 &lt; v10.0
          and item2 &lt; item10 correctly
        </li>
        <li>
          <strong>Type detection:</strong> Number, date, and boolean columns use appropriate
          comparators automatically
        </li>
        <li>
          <strong>Multi-sort:</strong> Hold Shift+click for multi-column sort, or use the preset
          button
        </li>
        <li>
          <strong>Sort stability:</strong> Randomize prices while sorted to see maintain/resort
          behavior
        </li>
      </ul>
      <div class="bg-default/50 text-muted rounded p-2 text-sm">
        ...
        <ul class="mt-1 ml-2 list-inside list-disc">
          ...
          <li>
            Sort by Status — sorts Active(1), Pending(2), Draft(3), Archived(4) via sortAccessor
          </li>

...
const arrIncludesAll: FilterFn<any> = (
  ...
  return !filterValue.some(
    (val: unknown) => !row.getValue<unknown[]>(columnId)?.includes(val),
  )
}
...
const inNumberRange: FilterFn<any> & {
  ...
  }) as FilterFn<any>,
  {
    resolveFilterValue: (val: [any, any]): readonly [number, number] => {
      ...
      let parsedMin
        = typeof unsafeMin !== 'number'
          ? parseFloat(unsafeMin as string)
          : unsafeMin
      let parsedMax
        = typeof unsafeMax !== 'number'
          ? parseFloat(unsafeMax as string)
          : unsafeMax
      let min
